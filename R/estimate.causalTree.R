...
gte.descendant.leaves <- function(parent, leaves) {
...
estimate.leaf.tau <- function(leaf.assignments, treat, control, Y, quant, leaves, leaf) {
  index <- which(leaf.assignments == leaf)
  index1 <- intersect(index, treat)
  index0 <- intersect(index, control)    
  tau <- quantile(Y[index1], quant) - quantile(Y[index0], quant)
  parent <- leaf
  while(is.na(tau)){
...
estimate.causalTree <- function(object, data, treatment, na.action = na.causalTree, quant)
{
 ...
 if (length(which(treatment == 0)) == 0 || length(which(treatment == 1)) == 0)
  ...
  for (leaf in unique_leaf) {
    ...
    causal_estimation[as.numeric(index)] <- estimate.leaf.tau(where, treat, control, Y, quant, leaves, leaf)
  }
